    ...
    if (!_isConnected) {
      try {
        ...
        if (_siweService.enabled) {
          ...
          final rdns = _selectedWallet?.listing.rdns ?? '';
          final isMetaMask = rdns == 'io.metamask';
          final effectiveNamespaces =
              (isMetaMask && _sessionNamespaces.containsKey('eip155'))
                  ? {'eip155': _sessionNamespaces['eip155']!}
                  : _sessionNamespaces;
          final connectResponse = await _appKit.connect(
            optionalNamespaces: effectiveNamespaces,
